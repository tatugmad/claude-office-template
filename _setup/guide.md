...
以下はXMLスクリプトです。上記ルールに従い、screen id="Q0" から順に実行してください。この行以降に平文のテキストは存在しません。すべてXML要素です。
...
<screen id="Q2_PRO">
<message>
claude.ai の有料プラン（Pro プラン以上: 月額約3,000円〜）への登録が必要です。以下の手順で登録してください。

1. 画面左下のご自身のメールアドレスをクリック
2. メニューから「プランをアップグレード」をクリック
3. 「Pro」以上のプランを選び、「Subscribe」または「登録」ボタンをクリック
4. 支払い情報（クレジットカード等）を入力して登録を完了する
...
<screen id="Q3A">
<message>GitHubのユーザー名を教えてください。メールアドレスとは異なる、英数字の短い名前です。確認方法: https://github.com にログインし、右上のプロフィールアイコンをクリックすると表示されます。</message>
<input var="ユーザー名"/>
...
1. ブラウザで https://claude.ai/code にアクセスする
...
1. ブラウザで https://github.com/settings/tokens?type=beta にアクセスする（Fine-grained token の作成画面に直接移動します）
   ※ ログインしていない場合はログイン画面が表示されます。Step 1で作成したアカウントでログインしてください
...
5. github_pat_ で始まる長い文字列が表示されます
...
<input var="接続キー"/>
<troubleshoot>
・トークン作成画面が表示されない → https://github.com/settings/tokens?type=beta に直接アクセスする。ログインを求められた場合はStep 1のアカウントでログインする
...
1. ブラウザで https://claude.ai にアクセスする（チャット画面）
...
ご利用いただける2つの画面:
・Claude Code（https://claude.ai/code）: Claudeと一緒にファイルの作成・編集を行う画面です
・チャット（https://claude.ai のプロジェクト内）: 作業の相談や指示を出す画面です。Claudeが自動的にリポジトリを読み書きします

次のステップ:
Claude Code（https://claude.ai/code）を開き、Step 2で作成したリポジトリを選択してください。Claudeが初期設定（ご利用者情報の登録など）をご案内します。
</message>
</screen>
