...
- **ref/ にリンクファイル（.ref.md）がある場合**: 参照先のソースコード用リポの `_tasks/` を GitHub API で確認し、指示ファイルの状態を把握する。`_tasks/review/` にあればレビューを実施する。レビュー完了後、本体を `_claude/tasks/done/YYYY-MM/` に保存し、ソースコード用リポから本体を削除、.ref.md も削除する。問題があれば修正した指示ファイルをソースコード用リポの `_tasks/pending/` に再配置する
...
2. リンクファイル（`.ref.md`）を `_claude/tasks/ref/` に配置する
